...
function colorFull (color, str, wp) {
  color = (colors[color] || colors.white).slice();

  var prefix = '\x1B[',
      total = str.length<=7 ? 7 : Math.ceil(str.length*1.2);
  str = new Array(Math.ceil((total-str.length)/2)+1).join(' ')+str+new Array(Math.floor((total-str.length)/2)+1).join(' ');

  return prefix + color[0] + 'm'+ (wp ? ( '[' + str + '] ') : str) + prefix + color[1] + 'm';
}
exports.each({
  'log': 'green',
  'warn': 'original',
  ...
  var prefix = colorFull(color, name.toUpperCase(), 1);
  exports[name] = function (str, level) {
    level = level ? parseInt(level) : 0;
    prefix = new Array(level+1).join('-')+prefix;
    ...
    var index = 0;
    args.unshift(prefix + str.replace(/\%[a-z]+/ig, function (s) {
      var type = exports.type(args.slice(index,1)).toLowerCase();
      if (!!~['object', 'array'].indexOf(type)) {
        return colorFull(colors.special, '%o');
      }
      if (!!~['number'].indexOf(type)) {
        return colorFull(colors.number,'%d');
      }
      return colorFull(colors[type] || colors.dft, '%s');
    }));
