...
function colorFull (color, str, style, align) {
  ...
  return [
      ...
      prefix, color[0], 'm',
      '[',
      align ? new Array(10 - str.length).join(' ') : '',
      str,
      ']',
      prefix, color[1], 'm',
...
exports.each({
  ...
  var prefix = colorFull(conf.color, name, conf.style, true);
  ...
  exports[name] = function (str) {
    ...
    var ret = '';
    if (exports.isInteger(lv)) {
      args.pop();
      ret = new Array(lv*4 + 1).join(' ');
    }
    if (!exports.isString(str)) {
      args.unshift(ret + prefix);
    } else {
      args.shift();
      var i = -1;
      str = ret + prefix + str.replace(/\%[a-zA-Z]+/g, function(i) {
        var type = exports.type(args[++i]).toLowerCase();
        if (!!~['object','array'].indexOf(type)) {
          type = 'special';
        }
        return colorFull(colors[styles[type] || 'white'], i);
      });
      args.unshift(str);
    }
